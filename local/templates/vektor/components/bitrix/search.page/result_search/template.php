...
<div class="container">
    <h1 class="inner__title">Результаты поиска по запросу:</h1>
<form action="" method="get" class="search-form search-form_big  search-results__search">
...
<?else:?>
	<input type="text" name="q" value="<?=$arResult["REQUEST"]["QUERY"]?>" class="search-form__input input" placeholder="Начните вводить слово" />
<?endif;?>
...
<?elseif(count($arResult["SEARCH"])>0):?>
	...
	<?foreach($arResult["SEARCH"] as $arItem):?>
        <div class="search-results__item">
            <a href="<?echo $arItem["URL"]?>" class="search-results__item-name"><?echo $arItem["TITLE_FORMATED"]?></a>
            <p class="search-results__item-desc"><?echo $arItem["BODY_FORMATED"]?></p>
        </div>
		<?if (
	...
	<?if($arResult["REQUEST"]["HOW"]=="d"):?>
		<a href="<?=$arResult["URL"]?>&amp;how=r<?echo $arResult["REQUEST"]["FROM"]? '&amp;from='.$arResult["REQUEST"]["FROM"]: ''?><?echo $arResult["REQUEST"]["TO"]? '&amp;to='.$arResult["REQUEST"]["TO"]: ''?>">
            <span><?=GetMessage("SEARCH_SORT_BY_RANK")?></span>
        </a>
        &nbsp;|&nbsp;
        <b><?=GetMessage("SEARCH_SORTED_BY_DATE")?></b>
	<?else:?>
        <span><?=GetMessage("SEARCH_SORTED_BY_RANK")?></span>
            ...
            <span><?=GetMessage("SEARCH_SORT_BY_DATE")?></span>
        </a>
	<?endif;?>
	</p>
<?else:?>
